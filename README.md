...
- Blocklyエディタの追加（メニュー→コマンド→「Blocklyエディタ...」。ブラウザで操作を組んで保存・再編集・削除。画像認識ブロックつきでカメラ連携スクリプトも作れる。詳細は `docs/BLOCKLY_EDITOR.md`）
- WakeSetupボタンの拡充（P/W/D/M/X/K＋USB状態表示。pico-wakeConにQ/R行が無いためキュー経路を廃止しS行に統一）
- キーボード入力の改善（ウィンドウが非前面のときは拾わない。OS全体に届く副作用を防止）
