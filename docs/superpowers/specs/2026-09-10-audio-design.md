...
## 10. 遅延の定量化と最低遅延の採用（2026-09-10 追加）

- 前提：ゲーム音はキャプチャボードからしか取れないため、入力は実質固定。入力のランキング・自動切替はしない。起動時に設定が空ならカメラ名から取込口を自動選択する（語の重なり2語以上。保存はせず毎回推定）。手動選択は残す。
- 一覧には試し開き時の `stream.latency` を推定表示（`番号: 名前 [est. XXms]`）。裏スレッドで絞り込み、番号管理は維持。
- デバイス自レートで開き、内部44.1kは維持（入力→`resample_poly`で44.1k化、出力→自レートへ変換。検知・録音は不変）。48kHz専用WASAPI機が選択肢に入る。CABLE実測でリサンプル0.35ms/塊（予算11.6msの3%）。
- 実測は押下→検知フロー（`core/audio_latency.py` の `measure_press`）：Aボタンを押してSwitchを鳴らし、立ち上がりをTap時刻で日付づけして中央値を出す。T0もT1も自時計のため loopback 配線は要らない。操作確認画面など押すと音が出る画面が前提。チャープ往復案は loopback を持つ人にしか使えないため不採用（未検証のまま削除）。
- 「遅延計測」ボタン：押下→検知を3回行い、入力の実測として記録・表示する。シリアル未接続では測れない。
- 「最速を選択」ボタン：出力のみ推定最小へ切り替える（加える形。既存の手動選択は不変）。
- ボタン押下音の新設は見送り（計測自体が押下を使うため、別途の音は不要）。
- 既知の限界：申告遅延は機器により fiction の場合がある。順位付けは実測を優先すること。
