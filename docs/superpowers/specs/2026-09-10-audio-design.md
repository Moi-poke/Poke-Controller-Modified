...
- 実測は `core/audio_latency.py`：選択中の出力へ給電コールバックでチャープ×5発→選択中の入力で相互相関検出→発射と検出を時刻近接で突き合わせ（見逃し・誤検出に引きずられない。窓外-50ms〜2sは不採用）。中央値を `実測 XXms` と表示。
- 「最速を選択」ボタン：実測優先・なければ推定で最小の出力を選ぶ（加える形。既存の手動選択は不変）。
- ボタン押下音の新設は見送り（押下→beep経路はシリアル遅延が混入し比較を濁す。チャープ直接計測の方が正確）。
- 既知の限界：申告遅延は機器により fiction（仮想ケーブルで申告93msに対し実測~15msを確認）。順位付けは実測を優先すること。
