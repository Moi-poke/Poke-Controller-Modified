...
ブロック： program（NAME＋DO、1個まで）、press（ボタン14種＋長さ＋待ち）、stick（L/R＋角度＋強さ＋長さ＋待ち）、wait（秒）。サブルーチン（定義＋呼出、引数あり）、コメント（`#` 生成）。繰り返し・条件・数値・変数は標準ブロックを使う。

スティック： 「スティック」ブロック（L/R・角度0〜360度・強さ0〜100%・長さ・待ち）で `self.press(Direction(Stick.L/R, 角度, magnification=強さ/100), ...)` を出す。編集画面の「スティック」区画で円パッドをドラッグすると角度・強さが決まり（数値欄とも連動）、`[選択中へ反映]`で選択中のブロックへ入る（選択が外れているときは最後に触ったブロック）。
