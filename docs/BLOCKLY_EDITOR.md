...
サンプル： `Commands/PythonCommands/BlocklySample*.py`（同名`.blockly.json`付き11種：基本・スティック・サブルーチン・画像認識・保持連打・変数フロー・出力・音検知・設定入力・時間制限・画像音声）。そのまま実行・再編集できる。画像認識系のテンプレは `controller_check/btn_a.png` を使う。音検知はAudio欄の入力が必要。なお `.py` は保存時に整形されるため、生成直後のコードと引用符等が変わることがある。

保存物： `Commands/PythonCommands/<保存名>.py`と同名`.blockly.json`の対。保存名は英字・数字・`_`（例: MyBlock）。`.blockly.json`は一覧を壊さない（`.py`のみ走査）。

タグ： プログラム欄のタグ（カンマ区切り、既定`blockly`）が `TAGS = [...]` として出る。一覧の絞り込み・タグ編集と連動する（GUIのタグ編集が最優先）。

ブロック： program（NAME＋DO＋タグ、1個まで）、press（ボタン14種＋長さ＋待ち）、stick（L/R＋パッド＋角度＋長さ＋待ち）、wait（秒）、経過時間、hold（押し続け）、holdEnd（離す）、finish（正常終了）、pressRep（連打）、表示（ログ出し）、スクショ、Discord通知。サブルーチン（定義＋呼出、引数あり）、コメント（`#` 生成）。繰り返し・条件・中断・比較・論理・否定・数値・四則・剰余・加算・乱数・テキスト・結合・変数は標準ブロックを使う。
...
時間制限： 「開始からの秒数」ブロック（値）で経過秒が取れる。`if 経過 > 3600 → finish` の形で時間切れ終了にする。使用時のみ `import time` と起点が自動で付く。

標準： 繰り返し・while・if・中断（break/continue）・比較・論理演算・否定・数値・四則演算・剰余・加算・乱数・テキスト・結合・変数が使える。カウンタは変数＋加算、ジグザグ等の偶奇は剰余、見つけたら抜ける形は中断で書く。
...
サブルーチン： 定義ブロック（名前＋引数カンマ区切り＋DO＋戻り値）をトップレベルに置き、呼出ブロック（名前＋値3つまで）で `self.名前(...)` として呼ぶ。値が必要なときは「呼んだ値」ブロックを使う。生成は `class BlocklyCmd` のメソッドになる（戻り値つきは注釈なし）。名前は英字・数字・`_`、重複・未定義呼出・循環（直接の再帰を含む）・引数の数/名の不一致は保存時に異常にする。`do()` の直接呼出は不可。定義内の画像認識も自動で `ImageProcPythonCommand` 切替の対象になる。

コメント： コメントブロック（TEXT欄）は `do()` やサブルーチン内に `# ...` として残る（複数行は分割、実行無影響）。作業用メモ（右クリックのコメント）は生成コードに影響しない。

設定： 「設定」「設定数値」「設定確認」ブロックで実行前に入力を求め、受け変数に入れる（選択肢・範囲数値・ON/OFF）。取消は終了する。受け変数名は英字・数字・`_`、数値は最小≦最大でないと保存できない。
