...
ブロック： program（NAME＋DO、1個まで）、press（ボタン14種＋長さ＋待ち）、stick（L/R＋パッド＋角度＋強さ＋長さ＋待ち）、wait（秒）。サブルーチン（定義＋呼出、引数あり）、コメント（`#` 生成）。繰り返し・条件・数値・変数は標準ブロックを使う。

スティック： 「スティック」ブロック（L/R・円パッド・角度・強さ・長さ・待ち）で `self.press(Direction(Stick.L/R, 角度, magnification=強さ/100), ...)` を出す。ブロック内の円パッド（48px）を直接ドラッグすると角度（度）・強さ（%）が決まり、角度・強さの数値欄と相互反映される（パッド操作がブロック移動に伝わらないよう分離している）。画面上部の仮想スティック区画は非表示（ブロック内蔵パッドへ移行、DOMは残す）。
