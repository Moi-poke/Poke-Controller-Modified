...
ブロック： program（NAME＋DO、1個まで）、press（ボタン14種＋長さ＋待ち）、stick（L/R＋パッド＋角度＋長さ＋待ち）、wait（秒）、経過時間、hold（押し続け）、holdEnd（離す）、finish（正常終了）、pressRep（連打）、表示（ログ出し）、スクショ、Discord通知。サブルーチン（定義＋呼出、引数あり）、コメント（`#` 生成）。繰り返し・条件・中断・比較・論理・否定・数値・四則・剰余・加算・乱数・テキスト・結合・変数は標準ブロックを使う。
