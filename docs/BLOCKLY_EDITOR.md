...
ブロック： program（NAME＋DO、1個まで）、press（ボタン14種＋長さ＋待ち）、stick（L/R＋パッド＋長さ＋待ち）、wait（秒）。サブルーチン（定義＋呼出、引数あり）、コメント（`#` 生成）。繰り返し・条件・数値・変数は標準ブロックを使う。

スティック： 「スティック」ブロック（L/R・円パッド・長さ・待ち）で `self.press(Direction(Stick.L/R, 角度, magnification=強さ/100), ...)` を出す。ブロック内の円パッド（48px）を直接ドラッグすると角度（度）・強さ（%）が決まる。編集画面の「スティック」区画の円パッド（大きめ）・角度・強さの数値欄とも連動し、`[選択中へ反映]`で選択中のブロックへ入る（選択が外れているときは最後に触ったブロック）。数値で決めたいときは区画の数値欄を使う。
