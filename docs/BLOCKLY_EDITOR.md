...
スティック： 「スティック」ブロック（L/R・円パッド・角度・長さ・待ち）で `self.press(Direction(Stick.L/R, 角度, magnification=1.0), ...)` を出す。ブロック内の円パッド（48px）を直接ドラッグすると8方向（45度刻み）にスナップし、強さは常に100%。グリップ（青点）に重ねるとカーソルがつかむ手に変わる。角度の数値欄と相互反映される（パッド操作がブロック移動に伝わらないよう分離している）。画面上部の仮想スティック区画は非表示（ブロック内蔵パッドへ移行、DOMは残す）。
