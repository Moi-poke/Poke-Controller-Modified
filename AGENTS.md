...
- `Window.py` — entry point, `PokeControllerApp`. Does `os.chdir(BASE_DIR)` at startup; resolve all resource paths from `BASE_DIR`, never from cwd. Owns only assembly (init/settings/exit/title); panels live in `ui/` (`camera_panel` / `serial_panel` / `command_panel` / `log_panel` / `audio_panel` mixins), execution lifecycle in `services/command_runner.py`, serial ownership in `services/serial_service.py`, audio ownership in `services/audio_service.py` (services are tkinter-free; `ui/` must not import `Window`; see `tools/check_core.py`). Stateless UI helpers live in `core/` (`CommandTags` / `CommandStats`) and at `SerialController/` root (`LogPane` / `WindowUtils` / `WindowGeometry`).
- `Commands/PythonCommands/` — user automation scripts, auto-discovered by `CommandLoader` via `Utility.importAllModules`. New command = new `.py` there with a class subclassing `PythonCommand` (inputs only), `ImageProcPythonCommand` (inputs + OpenCV vision), or `AudioPythonCommand` (inputs + audio detection via `waitTone`/`waitSound`) that sets a non-empty `NAME`. One broken file must not break the rest (loader imports per-module) — keep that behavior. Reload happens live via `CommandLoader.reload()`, so avoid module-level side effects. The `Commands.*` import paths (`Keys` / `PythonCommandBase` / `McuCommandBase` / `WakeLink` / `CommandVision` / `CommandAudio`) are frozen public API for these scripts (enforced by `task userapi`); internals live in `core/` (`serial/` for Sender+Arbiter+encoding, `transport/` for base+text_serial+registry).
